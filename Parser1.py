...
INT_VARIABLE : /(?!p)(?!f)[a-zA-Z_][a-zA-Z 0-9]*/
FLOAT_VARIABLE : /(?!p)f[a-zA-Z_][a-zA-Z0-9]*/
ENTIER : INT
FLOAT : /-?\d+\.\d+/
OPBINAIRE: /[+*\/&><]/|">="|"-"|">>"  
...
| (INT_VARIABLE |FLOAT_VARIABLE|POINTEUR_INT|POINTEUR_FLOAT) ("," (INT_VARIABLE | FLOAT_VARIABLE|POINTEUR_INT|POINTEUR_FLOAT))* -> liste_normale
...
t = parser.parse("""main(x,fy,z,piA){
    fy = fy+1.0;
    printf(fy);
    z=1;
    printf(z);
...
def pretty_printer_lhs(t):
    ...
    elif t.data == "lhs_pointeur_dereference":
        return "*" + t.children[0].children[0].value
    return "*"+t.children[0].value

def pretty_printer_expression(t):
    if t.data in ("exp_entier", "exp_int_variable", "exp_float", "exp_float_variable", "pointeur_int", "pointeur_float", "int_variable", "float_variable"):
        return t.children[0].value
    elif t.data in ("exp_pointeur_deref_int", "exp_pointeur_deref_float"):
        return "*" + t.children[0].value
    elif t.data == "exp_adresse":
        return "&"+ pretty_printer_lhs(t.children[0])
    elif t.data in ("exp_bin_int", "exp_bin_float","exp_pointeur"):
